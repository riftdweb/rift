    ...
    acceptedFiles.forEach((file) => {
      ...
      const upload = async () => {
        try {
          ...
          if (file.directory) {
            const directory = file.files.reduce((acc, file) => ({ ...acc, [getRelativeFilePath(file)]: file.file }), {});

            response = await uploadDirectory(selectedPortal, directory, encodeURIComponent(file.name), { onUploadProgress });
          } else {
            response = await uploadFile(selectedPortal, file.file, { onUploadProgress });
          }
  ...
  return (
    <Box>
      <Box className='home-upload-white'>
        ...
        {files.length > 0 && (
          <Box
            css={{ margin: '$3 0', border: '1px solid $gray500', backgroundColor: '$panel', borderRadius: '$3', overflow: 'hidden' }}>
            <Flex
              css={{
                padding: '$2 $3',
                gap: '$1',
              ...
              <Box css={{ flex: 1, textAlign: 'right' }}>
                Time
              </Box>
            </Flex>
            {files.map((file, i) => {
