export const skapps = [{
  hnsDomain: 'skyfeed.hns',
  title: 'SkyFeed',
  description: 'Decentralized SkyDB-based alternative to Twitter, YouTube and Instagram with a native iOS, Android and web app.',
  tags: ['Social media', 'Video']
}, {
  hnsDomain: 'skysend.hns',
  title: 'SkySend',
  description: 'An open source, highly secure, private and decentralized way to send and share your files.',
  tags: ['Secure', 'File sharing']
}, {
  hnsDomain: 'hackerpaste.hns',
  title: 'Hacker Paste',
  description: 'A paste bin for Skynet. Hacker Paste is a one-of-a-kind text snippet sharing tool, secure by design and built for the decentralized web.',
  tags: ['Pastebin', 'File sharing']
}, {
  hnsDomain: 'skygallery.hns',
  title: 'SkyGallery',
  description: 'SkyGallery is a basic media gallery Skapp that aims to provide an decentralized and opensource alternative to Imgur.',
  tags: ['Image sharing', 'Galleries']
}, {
  hnsDomain: 'skybrain.hns',
  title: 'SkyBrain',
  description: 'The place where magic happens and you are able to store/manage memories and emotions, like happens in the human brain.',
  tags: ['Social media']
}]
