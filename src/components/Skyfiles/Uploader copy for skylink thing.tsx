...
export function Uploader() {
      ...
      const upload = async () => {
        try {
          ...
          if (file.directory) {
            ...
            response = await uploadFile(selectedPortal, file, {
              onUploadProgress,
            })
          }
          ...
          onFileStateChange(file, {
            status: 'complete',
            url: getSkylinkUrl(selectedPortal, response.skylink),
          })
        } catch (error) {
          if (
            error.response &&
  ...
  return (
    <Box>
      <Box className="home-upload-white">
        <Box className="home-upload-split">
          ...
          >
            ...
            {directoryMode && (
              <Text>
                Please note that directory upload is not a standard browser
                feature and the browser support is limited. To check whether
                your browser is compatible, visit{' '}
                <a
          ...
          <Box className="home-upload-retrieve">
            <Box className="home-upload-text">
              ...
              <form
                className={classNames('home-upload-retrieve-form', {
                  invalid: skylink && !isValidSkylink(skylink),
                })}
      ...
      <Text>
        Upon uploading a file, Skynet generates a 46 byte link called a{' '}
        <strong>Skylink</strong>. This link can then be shared with anyone to
        retrieve the file on any Skynet Webportal.
      </Text>
    </Box>
  )
}
